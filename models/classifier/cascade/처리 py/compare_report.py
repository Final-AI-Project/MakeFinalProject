...
def build_model(name, num_classes, img_size_hint=224):
    ...
    if n in ["mobilenet","mobilenet_v3_large"]:
        ...
        rec = 224
    elif n in ["shufflenet_v2", "shufflenet_v2_x1_0"]:
        ...
        try:
            ...
            raise ImportError("GhostNet을 쓰려면 `pip install timm` 필요")
        m = timm.create_model("ghostnet_100", pretrained=True, num_classes=num_classes)
        rec = 224
    elif n in ["mobilevitv2", "mobilevitv2_100"]:
        try:
            import timm
        except ImportError:
            raise ImportError("MobileViT(v2)를 쓰려면 `pip install timm` 필요")
        m = timm.create_model("mobilevitv2_100", pretrained=True, num_classes=num_classes)
        rec = 256  # MobileViT(v2) 권장 입력
    elif n in ["efficientnet_b0"]:
        m = models.efficientnet_b0(weights=EfficientNet_B0_Weights.IMAGENET1K_V1)
        in_feat = m.classifier[1].in_features
        m.classifier[1] = nn.Linear(in_feat, num_classes)
        rec = 224
    else:
        raise ValueError("지원 모델: mobilenet_v3_large | shufflenet_v2_x1_0 | ghostnet | mobilevitv2_100")
...
def main():
    ...
    ap.add_argument("--mobilenet_weights", required=False)
    ap.add_argument("--ghostnet_weights", required=False)
    ap.add_argument("--mobilevitv2_weights", required=False)
    ap.add_argument("--shufflenet_v2_weights", required=False)
    ap.add_argument("--efficientnet_b0_weights", required=False)

    ap.add_argument("--mobilenet_name", default="mobilenet_v3_large")
    ap.add_argument("--ghostnet_name", default="ghostnet")
    ap.add_argument("--mobilevitv2_name", default="mobilevitv2")
    ap.add_argument("--shufflenet_v2_name", default="shufflenet_v2")
    ap.add_argument("--efficientnet_b0_name", default="efficientnet_b0")
    ...
    pairs = []
    if args.mobilenet_weights:
        pairs.append((args.mobilenet_name, args.mobilenet_weights))
    if args.ghostnet_weights:
        pairs.append((args.ghostnet_name, args.ghostnet_weights))
    if args.mobilevitv2_weights:
        pairs.append((args.mobilevitv2_name, args.mobilevitv2_weights))
    if args.shufflenet_v2_weights:
        pairs.append((args.shufflenet_v2_name, args.shufflenet_v2_weights))
    if args.efficientnet_b0_weights:
        pairs.append((args.efficientnet_b0_name, args.efficientnet_b0_weights))
