...
export default function Diary() {
  ...
  const [myPlants, setMyPlants] = useState<
    { label: string; value: string; species?: string }[]
  >([]);
  ...
  const fetchUserPlants = async () => {
    try {
      ...
      const apiUrl = await getApiUrl(API_ENDPOINTS.DIARY.PLANTS);
          ...
          if (refreshSuccess) {
            ...
            if (retryResponse.ok) {
              ...
              const plants = data.plants.map((plant: any) => ({
                label: `${plant.plant_name} (${plant.species})`,
      ...
      // 기본값 설정
      setMyPlants([
        { label: "초록이 (몬스테라)", value: "초록이", species: "몬스테라" },
        { label: "복덩이 (금전수)", value: "복덩이", species: "금전수" },
      ]);
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append("plant_nickname", selectedPlant || "");

      // 선택된 식물의 종류 찾기
      const selectedPlantData = myPlants.find(
        (plant) => plant.value === selectedPlant
      );
      if (selectedPlantData?.species) {
        formData.append("plant_species", selectedPlantData.species);
      }

      formData.append("weather", weather || "");
      formData.append("weather_icon", ""); // 날씨 아이콘은 나중에 추가
      ...
      const apiUrl = await getApiUrl(API_ENDPOINTS.DIARY.CREATE);
      const response = await fetch(apiUrl, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
        ...
        if (response.status === 401) {
          ...
          if (refreshSuccess) {
            ...
            if (retryResponse.ok) {
              ...
              setAiText(
                data.plant_reply ||
                  "식물이 답변을 준비 중입니다. 잠시 후 다시 확인해주세요."
              );
              setAiPreviewVisible(true);
              setSheetVisible(true);
              setIsSubmitted(true);
              Alert.alert("성공", "일기가 성공적으로 등록되었습니다!");
            } else {
      ...
      Alert.alert("오류", "일기 등록에 실패했습니다. 다시 시도해주세요.");
    } finally {
      setBusy(false);
    }
  };
