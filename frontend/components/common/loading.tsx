...
// 오버로드: 라우터와 작업 객체를 전달하는 경우
export function startLoading(router: RouterLike, job: LoadingJob): void;
// 실제 구현
export function startLoading(
  routerOrMessage: RouterLike | string,
  job?: LoadingJob
) {
  _cancelled = false;

  // 메시지만 전달된 경우
  if (typeof routerOrMessage === "string") {
    emit({ visible: true, error: null, message: routerOrMessage });
    return;
  }

  // 라우터와 작업 객체가 전달된 경우
  const router = routerOrMessage;
...
// 오버로드: 매개변수 없이 호출하는 경우
export function stopLoading(): void;
// 오버로드: 라우터와 옵션을 전달하는 경우
...
function CornersLoader({ size = 100, color = "#6B74E6" }) {
  ...
  return (
    ...
    >
      <Animated.View
        style={[loaderStyles.cornerWrap, { transform: [{ rotate: spin1 }] }]}
      >
        <View style={[loaderStyles.cornerBlock, { backgroundColor: color }]} />
      </Animated.View>
      <Animated.View
        style={[loaderStyles.cornerWrap, { transform: [{ rotate: spin2 }] }]}
      >
        <View style={[loaderStyles.cornerBlock, { backgroundColor: color }]} />
      </Animated.View>
      <Animated.View
        style={[loaderStyles.cornerWrap, { transform: [{ rotate: spin3 }] }]}
      >
        <View style={[loaderStyles.cornerBlock, { backgroundColor: color }]} />
      </Animated.View>
      <Animated.View
        style={[loaderStyles.cornerWrap, { transform: [{ rotate: spin4 }] }]}
      >
        <View style={[loaderStyles.cornerBlock, { backgroundColor: color }]} />
      </Animated.View>
    </Animated.View>
...
// expo-router가 이 파일을 라우트로 인식하지 않도록 빈 컴포넌트 export
