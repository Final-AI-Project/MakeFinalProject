"""
YOLOv8-Seg 모델 로딩 및 추론 유틸리티 (API 서비스용)
"""
import cv2
import numpy as np
...
def validate_image_size(image: np.ndarray, max_width: int = 4096, max_height: int = 4096) -> bool:
    """
    이미지 크기를 검증합니다.
    ...
    try:
        ...
        if width > max_width or height > max_height:
            logger.warning(f"이미지 크기가 너무 큽니다: {width}x{height}")
            return False
