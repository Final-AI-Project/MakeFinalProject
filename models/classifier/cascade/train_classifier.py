...
def main():
    ...
    labels_txt = Path(__file__).parent / "labels.txt"   # models/classifier/labels.txt
    ...
    # 학습
    best_model, best_acc = train(model, train_loader, val_loader, device, epochs=args.epochs, lr=args.lr)

    # 저장
    weight_dir = Path(__file__).resolve().parents[1] / "weight"   # models/weight/
