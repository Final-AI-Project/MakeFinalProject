...
│   ├── routes/                 # 라우터 통합 진입점
│   ├── pages/                  # 페이지 단위 라우터 (home, dashboard, info-room, diary-list)
│   ├── features/               # 기능 단위 라우터 (auth, plants, diary, medical, disease-diagnosis)
│   ├── services/               # 비즈니스 로직 및 모델 서버 연동
│   ├── db/                     # aiomysql 풀 및 쿼리 실행 유틸
│   ├── repositories/           # SQL 기반 DAO 함수
│   ├── schemas/                # Pydantic 입력/출력 모델
│   ├── clients/                # 외부 HTTP 연동 클라이언트
│   ├── core/                   # 설정, 로깅, 예외 핸들러
│   └── utils/                  # 공통 유틸리티
├── static/                     # 정적 파일 (이미지 등)
...
- 브라우저에서 `http://192.168.0.7:3000/docs` 접속
- API 문서가 정상적으로 로드되는지 확인
- 프론트엔드 앱에서 API 호출 테스트
...
- **호스트**: `--host 0.0.0.0`으로 설정하여 모든 네트워크 인터페이스에서 접근 가능하게 설정
...
- **Models Server**: http://localhost:5000 (AI 진단, LLM 답변)
- **Frontend**: React Native (Expo)
- **Database**: MySQL (AWS RDS)
- **Weather API**: 프론트엔드에서 외부 기상청 API 호출 후 백엔드로 전달
...
- **실시간 습도 데이터**: 디바이스에서 측정된 습도 정보를 DB에서 조회
- **기본값 처리**: 습도 데이터가 없는 식물은 50%로 표시
- **API 엔드포인트**: `/plants/humidity/{plant_id}`, `/plants/humidity/batch`
- **대시보드 통합**: `/home/plants/current` API에 습도 정보 포함

### 📝 **일기 작성/수정 시스템** (ENHANCED!)

- 사용자가 일기를 작성하면 AI가 식물의 관점에서 답변 생성
- **사진 업로드, 날씨 정보 처리 (프론트엔드에서 받은 데이터 우선), 해시태그 지원**
- **식물 건강상태, 병충해, 습도 정보를 LLM에 전달하여 맞춤형 답변 생성**
- 일기 수정시 AI 답변이 새로운 내용에 맞춰 재생성
- **프론트엔드에서 외부 기상청 API 호출 후 백엔드로 전달**
...
- **이미지 저장 개선**: 진단 이미지가 정상적으로 저장되고 표시됨
- **아코디언 UI**: 진단 목록에서 펼치기 전/후 모두 이미지 정상 표시
- **건강 상태 처리**: 건강한 진단 결과는 저장하지 않음
- **등록 버튼 비활성화**: 건강한 상태일 때 등록 버튼 비활성화
- **이미지 URL 통일**: 모든 페이지에서 일관된 이미지 URL 처리

### 🌤️ **날씨 정보 처리** (ENHANCED!)

- **프론트엔드에서 외부 기상청 API 호출 후 백엔드로 전달**
- **프론트엔드에서 받은 날씨 정보 우선 처리**
- 프론트엔드에서 날씨 데이터를 받지 못한 경우 기본값 사용
- 날씨 상태, 온도, 습도, 아이콘 URL 제공
- 수정시 새로운 날씨 정보로 업데이트
...
11. ✅ **식물 대화** (LLM 기반 대화)
12. ✅ **습도 모니터링** (실시간 습도 데이터, 아치형 바 표시)
13. ✅ **병충해 진단** (이미지 업로드, 진단 결과 저장, 건강 상태 처리)
