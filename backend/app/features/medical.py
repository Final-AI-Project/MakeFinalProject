...
async def get_medical_statistics(
    ...
    try:
        async with get_db_connection() as (conn, cursor):
            stats = await get_medical_stats(db=(conn, cursor), user_id=user["user_id"])
        return MedicalStatsResponse(**stats)
    except Exception as e:
        print(f"[DEBUG] 진단 통계 조회 오류: {e}")
        import traceback
        print(f"[DEBUG] 트레이스백: {traceback.format_exc()}")
        raise HTTPException(
            status_code=500,
...
async def get_plant_medical_diagnoses(
    ...
    try:
        ...
        print(f"[DEBUG] 식물별 진단 기록 조회 오류: {e}")
        import traceback
        print(f"[DEBUG] 트레이스백: {traceback.format_exc()}")
        raise HTTPException(
            status_code=500,
